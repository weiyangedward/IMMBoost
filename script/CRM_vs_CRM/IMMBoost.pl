...
=head1 Usage
  ...
  perl IMMBoost.pl CRMList Outdir Datadir crmGroupTable
...
# die "Usage: perl $0 CRM.lst Outdir\n" unless @ARGV==2;
...
while (<IN>)
{
   ...
   `perl $Bin/prepareModelAndData.pl $indir $outdir`;
}
close IN;
warn "Step1 is done!\n";
...
for my $crm (@crmNames)
{
   ...
   for (my $k=1;$k<=10;$k++)
   {
       ...
       {
           warn "trial$k fold$i\n";
           `perl $Bin/generateIMMScoreFeature.pl $crm $outdir $k $i`;
       }
   }
}
warn "fil group...\n";
`perl $Bin/filterGroupCRM.pl $datadir $outdir $crmGroupTable`;
warn "fil Dmel...\n";
`perl $Bin/filterGroupCRM.DmelTrainData.pl $datadir $outdir $crmGroupTable`;
warn "Step2 is done!\n";
...
for my $crm (@crmNames){
  warn "$crm\n";
  `perl $Bin/msIMMBaseline.pl $crm $outdir`;
}
...
{
  warn "$crm\n";
  `perl $Bin/msIMM_SVM.pl $crm $outdir`;
}
...
for my $crm (@crmNames){
  warn "$crm\n";
  `perl $Bin/msIMM_RF.pl $crm $outdir`;
}
...
{
  warn "$crm\n";
  `perl $Bin/kmerSVMFeature.pl $crm $outdir`;
}
...
{
  warn "$crm\n";
  `perl $Bin/kmerSVM.pl $crm $outdir`;
}
...
{
  warn "$crm\n";
  `perl $Bin/ensembleModel.pl $crm $outdir`;
}
